...
#	self.driver = DaguWheelsDriver()
	...
	### PWM Altitude Control ###
	# [velocity, esc1, esc2]
	self.up = np.array([0, 0, 4, 5])
...
#	self.sub_estop = rospy.Subscriber("/spaceship/estop", EstopCmd, self.estop, queue_size=1)
 ...
 #           self.driver.setWheelsSpeed(left=0.0,right=0.0)
	...
	vel_left = self.vel_left + self.altitude_gain * msg.vel_up
	vel_right = self.vel_right + self.altitude_gain * msg.vel_up
	
	### limit ###
	vel_left = min(self.throttle_limit, max(-1*self.throttle_limit, vel_left))
	vel_right = min(self.throttle_limit, max(-1*self.throttle_limit, vel_right))


	if self.showTime == 10:
		print("throttle: ", vel_left)
...
#        self.driver.setWheelsSpeed(left=vel_left,right=vel_right)
    ...
    def cdAltitudeCmd_pwm(self, msg):
	if self.shutDown:
	    print("!!!shut down!!!")
            self.shutDownAll()
	    self.up[1] = 0
	...
	throttle = self.up[1] + (int)(self.altitude_gain * msg.vel_up)
#	throttle = min(150, max(-150, throttle))
	
	print("throttle = ", throttle)

	self.setup2(throttle, self.up)
	self.set_velocity(self.up)
