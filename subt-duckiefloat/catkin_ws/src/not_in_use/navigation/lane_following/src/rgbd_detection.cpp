...
WallDetection::WallDetection(ros::NodeHandle &n){
  ...
  // Publisher
  pc2 = nh.advertise<sensor_msgs::PointCloud2> ("/pc", 1);
  ...
  // Subscriber
  sub_cloud = nh.subscribe("/camera/depth_registered/points", 1, &WallDetection::cbCloud, this);
...
void WallDetection::cbCloud(const sensor_msgs::PointCloud2ConstPtr& cloud_msg){
  ...
  PointCloudXYZRGB::Ptr cloud_XYZ(new PointCloudXYZRGB);
  ...
  pcl::transformPointCloud(*cloud_XYZ, *cloud_XYZ, transform_matrix_1);
...
void WallDetection::pcl_preprocess(const PointCloudXYZRGB::Ptr cloud_in, PointCloudXYZRGB::Ptr cloud_out){
  PointCloudXYZRGB::Ptr pcl_filter1(new PointCloudXYZRGB);
  PointCloudXYZRGB::Ptr pcl_filter2(new PointCloudXYZRGB);
  float dis, angle = 0;
  int num = 0;
  for (int i=0 ; i <  cloud_in->points.size() ; i++)
  {
    ...
    //bool is_in_range =  dis >= range_min && dis <= range_max && 
    //                    angle >= angle_min && angle <= angle_max &&
    //                     cloud_in->points[i].z <= height_max;
    ...
    {
      pcl_filter1->points.push_back(cloud_in->points[i]);
      num ++;
    }
  }

  // pcl_filter1->width = num;
  // pcl_filter1->height = 1;
  // pcl_filter1->points.resize(num);
  ...
  // seg.setAxis(axis);
  // seg.setEpsAngle(  45.0f * (PI/180.0f) );
  // seg.setInputCloud (pcl_filter1);
  // seg.segment (*inliers, *coefficients);

  // extract.setInputCloud (pcl_filter1);
  ...
  // extract.filter (*pcl_filter2);

  *cloud_out = *pcl_filter1;

  /*
  pcl_filter1->clear();
  seg.setModelType (pcl::SACMODEL_PLANE);   // pcl::SACMODEL_PLANE   SACMODEL_PERPENDICULAR_PLANE
  seg.setMethodType (pcl::SAC_RANSAC);
  seg.setMaxIterations (20);               
  seg.setDistanceThreshold (0.05);
  seg.setInputCloud (pcl_filter2);
  seg.segment (*inliers, *coefficients); 

  extract.setInputCloud (pcl_filter2);
  extract.setIndices (inliers);
  extract.setNegative (false);
  extract.filter (*pcl_filter1);
  //pcl::concatenateFields (cloud_out,pcl_filter1,cloud_out);
  *cloud_out += *pcl_filter1;
  */
  sensor_msgs::PointCloud2 object_cloud_msg;
  toROSMsg(*cloud_out, object_cloud_msg);
